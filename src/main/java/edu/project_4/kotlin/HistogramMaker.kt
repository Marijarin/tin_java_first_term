...
class HistogramMaker {
    ...
    fun makeHistogram2() {
        val x1 = ThreadLocalRandom.current().nextDouble(-1.77, 1.77)
        val y1 = ThreadLocalRandom.current().nextDouble(-1.0, 1.0)
        val first = Point(x1, y1, RGB(0.8, 0.0, 0.8))
        points.clear()
        points.add(first)
        ...
        for (i in 1..iterations) {
            ...
            next = when {
                ...
                i % 3 == 0 -> next.copy(color = RGB(0.0, 0.0, 0.0))
    ...
    fun makeHistogram3() {
        ...
        for (i in 1..iterations) {
            ...
            next = next.copy(color = RGB(next.color.r + 0.03, next.color.b + 0.08, next.color.g + 0.04))
            points.add(next)
        }
    }
